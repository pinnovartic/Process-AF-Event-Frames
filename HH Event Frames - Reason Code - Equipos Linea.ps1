...
$TimeQuerySt = ConvertFrom-AFRelativeTime -RelativeTime "26-mar"
Write-Host "Fecha Inicio: " $TimeQuerySt
#$TimeQuerySt = $TimeQuerySt.ToLocalTime() 
$TimeQueryEt = ConvertFrom-AFRelativeTime -RelativeTime "27-mar"
...
$credentials = New-Object System.Management.Automation.PSCredential ($Str_AFUser, $secure_pass)
    try{
        ...
        $AFTempElement = Get-AFElement -AFDatabase $AFDB -Name "Estado Equipos"
        $AFRootElement = Get-AFElement -AFElement $AFTempElement -Name "Linea RE010"
    }catch { 
        $e = $_.Exception
        $msg = $e.Message
